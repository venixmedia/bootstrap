...
function lupercalia_scripts_styles() {
	...
	// Lupercalia Theme 
	wp_enqueue_script( 'lupercalia-script', get_template_directory_uri() . '/js/scripts.js', array('jquery'), '', true);
	...
	// bxSlider
	wp_enqueue_script( 'bxslider-scripts', get_template_directory_uri() . '/js/jquery.bxslider.min.js', array('jquery'), '', true);
	wp_enqueue_style( 'bxslider-style', get_template_directory_uri() . '/js/jquery.bxslider.css');
	
	// Maginific PopUp
	wp_enqueue_script( 'magnificpopup-scripts', get_template_directory_uri() . '/js/magnificpopup.min.js', array('jquery'), '', true);
	wp_enqueue_style( 'magnificpopup-style', get_template_directory_uri() . '/js/magnificpopup.css');
...
function lupercalia_social_links_callback() {
	...
	if ( !empty($option['contact_facebook_field']) || !empty($option['contact_flickr_field']) || !empty($option['contact_googleplus_field']) || !empty($option['contact_instagram_field']) || !empty($option['contact_linkedin_field']) || !empty($option['contact_pinterest_field']) || !empty($option['contact_twitter_field']) || !empty($option['contact_tumblr_field']) || !empty($option['contact_vimeo_field']) || !empty($option['contact_youtube_field']) ) : 
		...
		if ( !empty($option['contact_tumblr_field']) ) :
			echo "<li class='tumblr'><a href='" . esc_url($option['contact_tumblr_field']) . "' title='" . esc_html__('Tumblr', 'lupercalia') . "' target='_blank'></a></li>";
		endif;		
...
function lupercalia_slider_script ( $args = array() ) {
	...
	$script = "<script>jQuery(document).ready(function() {
			   ...
			   jQuery('.bxslider-" . $slider_id . "').bxSlider({
					minSlides:" . $mobile_items .",
					maxSlides:" . $visible_items . ",
					slideWidth: " . $width / $visible_items . ",
					slideMargin: 5,
					auto:" . $auto_play . ", 
					useCSS: false,
					adaptiveHeight: true,
...
function lupercalia_slider ( $args = array() ) {
	...
	$script = lupercalia_slider_script( array ( "visible_items" => $visible_items, "auto_play" => $auto_play, "mobile_items" => $mobile_items, "slider_id" => $slider_id, "width" => $width ) );
	
	echo $script; ?>
		
	<ul class="bxslider-<?php echo $slider_id; ?>">
	...
	if ( $the_query->have_posts() ) :
		...
		if ($option['relatedposts_slider_field']['relatedposts_slider_hide_field'] != 'yes') :
			...
			if ($option) :
				...
				$args = array ( 'num_posts' => 5, 'visible_items' => 2, 'category_id' => lupercalia_post_category(), 'mobile_items' => 2, "auto_play" => 'false', 'orderby' => 'rand', 'post__not_in' => array($post->ID), 'width' => 700 );
...
function lupercalia_front_page_slider() {
	...
	if ($option['frontpage_slider_field']['frontpage_slider_hide_field'] != 'yes') :
		...
		if ($option) :
			...
			$args = array ( 'visible_items' => 3, 'mobile_items' => 2, "auto_play" => 'false', 'orderby' => 'ASC', 'width' => 1024 );
...
function lupercalia_blog_slider() {
	...
	if ($option['home_slider_field']['home_slider_hide_field'] != 'yes' and is_home() and !is_paged()) :
		...
		if ($option) :
			...
			$args = array ( 'visible_items' => 1, 'mobile_items' => 1, "auto_play" => 'true', 'orderby' => 'ASC', 'width' => 700 );
...
function lupercalia_front_page_branding_section() {
	...
	$script = lupercalia_slider_script( array ( "visible_items" => 1, "auto_play" => 'true', "mobile_items" => 1, "slider_id" => $slider_id, "width" => 1600, "pager" => 1 ) ); 
				...
				<?php for ($i = 0; $i < $arrayimg; $i++) : ?>						
					...
					<li>
						...
						<?php if ( $option['frontpage_branding_field']['slider_image'][$i] ) : ?>
							...
							<div class="amazingcarousel-image">
								<a href="<?php echo esc_url($option['frontpage_branding_field']['slider_url'][$i]); ?>" target="<?php echo $option['frontpage_branding_field']['slider_target'][$i]; ?>"><img src="<?php echo esc_url($option['frontpage_branding_field']['slider_image'][$i]); ?>" /></a>
							</div> <!-- .amazingcarousel-image -->
						
						<?php else : ?>
								
							<div class="amazingcarousel-image">
								<img src="<?php echo get_template_directory_uri(); ?>/imgs/slider-sample.png" />
							</div> <!-- .amazingcarousel-image -->
					...
					<ul class="bxslider-<?php echo $slider_id; ?>">
						...
						<li>
							...
							<div class="amazingcarousel-text">
								...
								<div class="amazingcarousel-title">Slider 1 Title</div> <!-- .amazingcarousel-title -->
						...
						<li>
							...
							<div class="amazingcarousel-text">
								...
								<div class="amazingcarousel-title">Slider 2 Title</div> <!-- .amazingcarousel-title -->
						...
						<li>
							...
							<div class="amazingcarousel-text">
								...
								<div class="amazingcarousel-title">Slider 3 Title</div> <!-- .amazingcarousel-title -->
...
function lupercalia_post_gallery($output, $attr) {
	...
	$script = lupercalia_slider_script( array ( "visible_items" => $columns, "auto_play" => 'true', "mobile_items" => 1, "slider_id" => $slider_id, 'width' => 692 ) );

    // Here's your actual output, you may customize it to your need
	$output = $script;
	$output .= "<ul class='bxslider-" . $slider_id . "'>";
